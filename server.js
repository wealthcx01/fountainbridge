...
app.prepare().then(() => {
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    // The browser's upgrade is answered at once, and the box is dialled behind it.
    //
    // FB-196 tried the other order — dial the box, collect the office's opening burst, then answer
    // the browser — and Railway refused it. The edge will not wait: it closes the raw socket before
    // a reply, and the studio's own log said so plainly once it could
    // (`the box closed it before the office answered`, and `WebSocket was closed before the
    // connection was established`, which is what happens when the browser's socket goes away while
    // the box is still being dialled). An upgrade has to be answered promptly or not at all.
    //
    // That left FB-195's problem: after the 101 the studio was silent while it dialled the box, and
    // the edge cuts a connection that carries nothing. The fix was not in this file. The studio's
    // container was in **Singapore** and the venture box is in Nuremberg, so the dial took about
    // 650ms — while the edge cuts at about 375ms. Moved to eu-west it takes about 52ms, measured
    // through `/venture/<id>/office-ready`: 832ms before, 84ms after. The silence now fits inside
    // the budget with room to spare.
    //
    // Two things still narrow the window as far as it will go, and both are cheap:
    wss.handleUpgrade(req, socket, head, (client) => {
      ...
      // One: a ping, straight away, so the connection carries traffic from its first moment rather
      // than waiting on somewhere else. It is protocol-level, so the office app never sees it and
      // has nothing to misparse, and the browser answers automatically. Repeated every twenty
      // seconds, which also holds the socket through the quiet spells — and those are most of the
      // day, since a venture office is still most of the time.
      const beat = () => {
      ...
      //
      // FB-194: this used to be four handlers pointing at one silent function, so the only line in
      // the log was `WebSocket was closed before the connection was established` — the CONSEQUENCE
      // of shutting down mid-dial. It reads exactly like the box being unreachable. The box was
      // never unreachable, and that one misleading sentence is most of what FB-163 got wrong.
      const shut = (why) => {
        if (ended) return;
        ended = why;
      ...
      upstream.on('open', () => {
        // Two: the studio asks for the room itself rather than waiting to be asked.
        //
        // The office says nothing at all until it receives `webviewReady`, and the browser's copy of
        // that has to cross the whole distance twice before it arrives. Sending it here starts the
        // room coming the moment the box is connected — so the burst is on its way while the
        // browser's own handshake is still in flight, and the socket is carrying real traffic
        // within tens of milliseconds instead of hundreds.
        //
        // `webviewReady` is the one message that ever travels in this direction
        // (OFFICE_ALLOWED_CLIENT_MESSAGES), so this is the studio saying exactly what a browser
        // would say and nothing a browser could not. The browser's own copy is forwarded when it
        // arrives; the box answers it again, and a second burst costs nothing anyone can feel.
        upstream.send(JSON.stringify({ type: 'webviewReady' }));
