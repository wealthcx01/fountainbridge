...
**Founder identity, defined once:** wherever these docs say the founder's "Gmail," it means a **Google account assigned by Bruntsfield Capital and managed/run by the founder themselves, on the shared Bruntsfield Workspace** (`@bruntsfield.capital`, e.g. `ross@bruntsfield.capital`) — never a personal consumer @gmail.com. *(Amended 2026-07-22: founders sit on the shared `@bruntsfield.capital` Workspace until their venture spins out — each still gets their own Hetzner VPS + lane, D1. Previously each founder was on a per-venture domain, e.g. `ross@thereset.com`.)* One identity serves as: studio login (Google OAuth, D4), the founder's human mailbox, and the org-internal identity that makes Google's internal-app OAuth exemption apply cleanly (see `docs/research-gtm.md` §1). This resolves the v3 tension between "founders on their own Gmail" and the research findings — the founder's *own* account simply lives on venture infrastructure.
